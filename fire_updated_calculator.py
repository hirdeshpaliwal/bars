...
def render():
    ...
    if st.button("🚀 Calculate FIRE Scenarios", use_container_width=True):
        ...
        with col1:
            st.metric("LeanFIRE (1x)", f"₹{corpus_values['LeanFIRE (1x)']/10000000:.2f} Cr", delta="Minimum")
            
        with col2:
            st.metric("FIRE (1.25x)", f"₹{corpus_values['FIRE (1.25x)']/10000000:.2f} Cr", delta="Comfortable")
            
        with col3:
            st.metric("FatFIRE (3x)", f"₹{corpus_values['FatFIRE (3x)']/10000000:.2f} Cr", delta="Luxurious")
        ...
        fig.update_layout(
            ...
            yaxis2=dict(title="Annual Expense (₹ Lakhs)", overlaying='y', side='right', showgrid=False),
        ...
        **Verdict:** {achievable_status}
        ...
        coast_fire_target_retirement = exact_needed_corpus
        ...
        with c_col1:
            st.metric("Coast FIRE Target (at Ret. Age)", f"₹{coast_fire_target_retirement/10000000:.2f} Cr")
        with c_col2:
            st.metric("Required Corpus Today", f"₹{coast_fire_needed_today/10000000:.2f} Cr")
        with c_col3:
        ...
        if coast_status_delta >= 0:
            st.success(f"🎉 **You are Coast FIRE!** \n\nIf you stop saving today, your current corpus of **₹{current_corpus/100000:.2f} L** is projected to grow to **₹{coast_fire_projected_corpus/10000000:.2f} Cr** by age {retirement_age}, exceeding your requirement.")
        else:
